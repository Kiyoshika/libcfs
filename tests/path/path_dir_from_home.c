...
#define EXPECTED_WINDOWS_PATH "\\path\\from\\home\\"
#define EXPECTED_UNIX_PATH "/path/from/home/"
...
int main()
{
    ...
    dynamic_path_2[2] = strdup("home");

    struct cfs_result_string_t home_result = cfs_path_home_d();
    char home_buffer[256] = {0};
    cfs_path_home_s(home_buffer, 256);

    struct cfs_result_string_t result = vcfs_path_dir_from_home_d(false, 0);
    assert(result.info.is_error == true);

    result = vcfs_path_dir_from_home_d(false, 3, "a", "b", "");
    assert(result.info.is_error == true);
    assert(result.info.error_type == ECFS_ERR_INVALID_ARG);

    result = vcfs_path_dir_from_home_d(3, "path", "from", "home");
    assert(result.info.is_error == false);

    ASSERT_PATH_VARARGS_D(result);

    char local_buffer[256] = {0};
    struct cfs_result_size_t result2 = vcfs_path_dir_from_home_s(local_buffer, 256, 3, "a", "b", "");
    assert(result2.info.is_error == true);
    
    result2 = vcfs_path_dir_from_home_s(local_buffer, 256, 3, "path", "from", "home");
    ...
    result = cfs_path_dir_from_home_d(3, (const char**)dynamic_path_1);
    assert(result.info.is_error == true);

    result = cfs_path_dir_from_home_d(3, (const char**)dynamic_path_2);
    assert(result.info.is_error == false);
    CFS_IMPL_WIN_OTHER(
    {
        char* buffer = calloc(strlen(home_result.value) + strlen(EXPECTED_WINDOWS_PATH) + 1, sizeof(char));
